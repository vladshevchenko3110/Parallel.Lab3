...
void mxvComparison(int loadmatrixsize) {
    ...
    vector<double> result(loadmatrixsize*loadmatrixsize);
    ...
    for (int i = 0; i < loadmatrixsize; ++i) {
        for (int j = 0; j < loadmatrixsize; ++j) {
            double element = loadedMatrix[i][j];
            if (loadedMatrix[i][j]!=0) {
                result.push_back(loadedMatrix[i][j]);                
            }
        }
    }
    ...
    vector<double> ompresult(loadmatrixsize * loadmatrixsize);
    ...
    int ompnonZeroCount = 0;

    Timer t2;
#pragma omp parallel for    
    for (int i = 0; i < loadmatrixsize; ++i) {
        for (int j = 0; j < loadmatrixsize; ++j) { 
            double element = omploadedMatrix[i][j];
            if (omploadedMatrix[i][j]!=0){
                ompresult.push_back(omploadedMatrix[i][j]);
    ...
    ompnewOutFileResult.write(reinterpret_cast<char*>(ompresult.data()), ompresult.size() * sizeof(double));
