...
private:
    ...
    using second_t = chrono::duration<double, std::ratio<1, 1000> >;
...
void msearchComparison(int loadmatrixsize) {
    ...
    cout << "Number of lines matching the condition: " << count << endl;
    ...
    newOutFileResult.write(reinterpret_cast<char*>(&count), sizeof(double));
    ...
    int chunk = 100;
    Timer t2;
#pragma omp parallel for schedule(dynamic,chunk) num_threads(10) reduction(+:count1)
    for (int i = 0; i < loadmatrixsize; ++i) {
        for (int j = 0; j < loadmatrixsize; ++j) {
            if (loadedMatrix[i][j] > 0) {
                count1++;
    ...
    cout << "Number of lines matching the condition: " << count1 << endl;
    ...
    ompnewOutFileResult.write(reinterpret_cast<char*>(&count1), sizeof(double));
...
int main(int argc, char* argv[])
{
    ...
    msearchComparison(stoi(argv[1]));
    msearchComparison(stoi(argv[2]));
    msearchComparison(stoi(argv[3]));
    msearchComparison(stoi(argv[4]));
    msearchComparison(stoi(argv[5]));
    msearchComparison(stoi(argv[6]));
    msearchComparison(stoi(argv[7]));
    msearchComparison(stoi(argv[8]));
    msearchComparison(stoi(argv[9]));
    msearchComparison(stoi(argv[10]));
